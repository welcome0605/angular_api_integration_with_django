...
export class HomeComponent implements OnInit {
    ...
    addCompany() {
        ...
        document.getElementById('componies').appendChild(iDiv).appendChild(iButton).
        setAttribute('Style', 'width: 200px !important;height: 200px !important;border-radius: 50%; margin-top:50px;');
    }

    removeCompany() {
        const length = document.getElementById('componies').childElementCount;
        console.log(length);
        if(length === 0) { return; }
        const companies = document.getElementById('componies');
